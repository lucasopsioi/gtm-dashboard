...
> 100 shipped releases and counting. Built entirely on personal time and equipment; contains no employer code or data. All demo data is synthetically generated with a fixed seed (fully reproducible).
...
Six eval-driven iterations, same 30 questions, human-reviewed grading:
...
| **E** | **release-86 orchestration refinements + provider model refresh** | **80.0%** | **0 — pass bar met (≥80% with zero red lines)** |
...
Electron + a pure-function analytics core (**74 test files** in this repo's suite, all green — `npm test`), streaming responses, per-request token/latency budgets tuned for on-device inference, bilingual UI (中/EN runtime toggle). `node scripts/make-demo-data.js` regenerates the full synthetic world (43k rows, 12 countries, deliberately embedded edge cases: EOL tail inventory, pre-launch demo units, late-reporting channels, seasonality).
